...
const CampaignDetails = ({ params }) => {
  ...
  const fetchCampaignDetails = async () => {
    if (contract && title) {
      try {
        // Assuming there's a method to get campaign ID by title
        const campaignData = await contract.getCampaignByTitle(title);
        setState({
          owner: campaignData[0],
          title: campaignData[1],
          description: campaignData[2],
          target: ethers.formatUnits(campaignData[3].toString(), 'ether'),
          deadline: campaignData[4],
          amountCollected: ethers.formatUnits(campaignData[5].toString(), 'ether'),
          image: campaignData[6],
          pId: campaignData[7], // Make sure you return the ID as well
        });
        fetchDonators();
      } catch (error) {
  ...
  }, [contract, title]); // Use title in the dependency array
          ...
          <div>
            ...
            <div className="mt-[20px] flex flex-row items-center flex-wrap gap-[14px]">
              <div className="w-[52px] h-[52px] flex items-center justify-center rounded-full bg-[#2c2f32] cursor-pointer">
                <Image src="/thirdweb.png" alt="user" className="w-[60%] h-[60%] object-contain" />
              </div>
              <div>
